...
getFundingOrder <- function(forecast, price, premium, trader, kMax) {
  if (forecast > premium) {
    print("BUY")
    # Return Buy order
    return(list(trader, price*(1-kMax)))
  } else {
    print("SELL")
    # Return Sell order
    return(list(trader, -1*price*(1+kMax)))
  }
}
```
...
getOrder <- function(trader, premia, perp_prices, t, bias, close_position_probability, sigmaE) {
  ...
  if (getAgentParameter(trader, "Side") == 0) {
    ...
    if (runif(1) < bias) {
      ...
      return(getFundingOrder(forecast, price_forecast, premia[t], trader, k))
    }
...
```{r message=FALSE, warning=FALSE}
perp_prices <- window(spot_price, start = 0, end = 0.250)

premia <- perp_prices - spot_price + runif(250, 0, 1)

trader <- createAgent(1, 1, 1, 0.25, 0, 0.005)
...
order <- getOrder(trader, premia, perp_prices, t = t, bias, close_position_probability, sigmaE = sigmaE)[2]

print("Perp price at t: ")
perp_prices[t]
print("Order at t: ")
order
```
